...
- **文件要小**：单文件超过 2000 行考虑拆分，3000 行是硬上限。一个文件长成"卷轴"，说明职责早就该拆了
...
- **分层选工具**：
  - **单元测试**：用语言原生框架——Go 用 `go test`、TS/前端用 Vitest、Java 用 JUnit、Python 用 pytest。语言自身生态成熟的，不引入额外语言
  - **API 层测试与端到端测试**：优先用 Ruby 编写自动化脚本（HTTP 调用、流程编排、断言），并输出 JSON 和 Markdown 两种格式的测试报告；尽量不用 Python 写这类脚本
  - **浏览器 E2E**：前端项目可用 Playwright，属于前端工程自身生态，不受 Ruby 约束
...
## 版本控制
详见 [version-control.md](version-control.md) — 分支策略、语义化版本、CHANGELOG、Product Overview、Architecture 文档、完整 Git 工作流。

## 产品与界面设计
- **开工前先判断项目阶段**：AI 接手任务后必须先判断当前仓库是新项目、半成品还是既有项目迭代；判断不清时按新项目处理，先确认规则再实现
- **新项目先生成协作规则与架构文档**：新项目或规则缺失时，先生成/更新 `AGENTS.md`、`CLAUDE.md`、`PRODUCT_OVERVIEW.md`、`ARCHITECTURE.md` 或等价文档，写清产品定义、技术栈、模块边界、数据流、设计规范、UI token、版本和验收标准
...
- **架构文档必须持续维护**：新项目必须创建根目录 `ARCHITECTURE.md`；技术栈、模块边界、服务通信、数据存储、接口协议、认证授权、部署拓扑或关键集成发生变化时，必须同步更新。
- **关键决策必须落文档**：架构、数据库、UI 规范、设计 token、多语言、部署和验收标准必须写入 `ARCHITECTURE.md`、`docs/`、`AGENTS.md`、`CLAUDE.md` 或等价仓库文档；不要只放在 memory 或聊天记录里
