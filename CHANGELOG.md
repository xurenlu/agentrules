...
### Added
- `ai-guardrails.md` 新增「不要探测凭据文件」：诊断远程连接要跑带 `BatchMode=yes` 的真实命令读 stderr，不得用 `cat` / `head` / 重定向去探测私钥、`.env`、token 文件是否可读。
  - 起因是实际撞到的一次：Agent 为了诊断 ssh，写出 `cat ~/.ssh/id_ed25519 >/dev/null; echo "read_exit=$?"`。它既没提供额外信息（真实命令的报错就能区分认证 / 网络 / 主机密钥三种原因），又离泄漏只差一步（`>/dev/null` 是唯一挡住内容的东西，这类命令常被复制改写），还白花用户一次审批确认。
  - 同时给出替代手法：需要元信息用 `ls -l` / `stat`；判断有没有 agent 转发只读 `SSH_AUTH_SOCK`；判断 API key 能不能用就发一个最小真实请求看状态码。
  - 被拦之后想办法绕过审批是方向错误——该改的是命令，不是闸。
...
### Changed
- 数据库迁移幂等性升级为硬约束：仅当目标数据库确实无法支持幂等实现时，才允许非幂等迁移；必须记录限制、前置条件、一次性执行保护和安全重试/恢复方案，并在生产执行前人工审批。
...
### Added
- AI 协作红线新增“需求文本不等于产品文案”硬约束：禁止把面向 AI/开发者的需求、Prompt、验收标准、实现备注及运维元数据直接展示给最终用户；确需展示时必须先判断必要性并改写为面向用户目标与操作场景的产品语言。
