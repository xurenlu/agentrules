...
- AI 协作红线：文件编辑方式、破坏性操作确认、共享分支保护、如实汇报、需求文本与产品文案边界，以及嵌套 `AGENTS.md` 的作用域和生成文件保护；required 文档，所有画像自动包含
...
- 数据库规范：迁移、查询、同步、索引、回滚和 CI/CD 集成；迁移默认必须幂等，只有数据库确实无法支持时才能走受控例外
...
- 工具与发布：Ruby 脚本既能按项目画像生成单文件规则，也能扫描目标项目并生成根目录 + 模块级的分层 `AGENTS.md`；支持通用、React、Go API、Go + React 模板，已有规则文件绝不覆盖；校验脚本输出 JSON 和 Markdown 报告，CI 检查规则结构、版本一致性、生成产物同步和脚手架重复执行安全性；通过主分支门禁的版本 tag 才能自动发布最终精简版 `CLAUDE.md`
