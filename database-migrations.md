...
- **幂等性**：迁移必须可以安全重复执行，优先使用 `IF NOT EXISTS` / `IF EXISTS`、条件更新、唯一约束、迁移状态表或事务保护。只有目标数据库的语法或行为确实无法支持幂等实现时，才允许非幂等迁移；迁移文件或 ADR 必须写明无法幂等的数据库限制、执行前置条件、一次性执行保护、失败后的安全重试/恢复步骤，并在 prod 执行前人工审批。
